...
public class UserResource {
    ...
    @Produces(BeeterMediaType.GROUPTALK_AUTH_TOKEN)
    public Response registerUser(@FormParam("loginid") String loginid, @FormParam("password") String password, @FormParam("email") String email, @FormParam("fullname") String fullname, @Context UriInfo uriInfo) throws URISyntaxException {
        ...
        return Response.created(uri).type(BeeterMediaType.GROUPTALK_AUTH_TOKEN).entity(authenticationToken).build();
    }
    @Path("/{id}")
    @GET
    @Produces(BeeterMediaType.GROUPTALK_USER)
    ...
    @Path("/{id}")
    @PUT
    @Consumes(BeeterMediaType.GROUPTALK_USER)
    @Produces(BeeterMediaType.GROUPTALK_USER)
